...
import { aboutValidator } from '../validations/about'

const aboutRoutes = Router()

aboutRoutes.post('/', aboutValidator, (req: Request, res: Response) => {
...
aboutRoutes.put('/', aboutValidator, (req: Request, res: Response) => {
  const errors = validationResult(req)
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() })
  }
